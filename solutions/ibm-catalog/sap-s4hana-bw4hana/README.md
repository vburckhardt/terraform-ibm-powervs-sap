...
- Creates and configures **one** PowerVS instance for **SAP NetWeaver** based on best practices, which **hosts the PAS and ASCS instance**.
...
2. **This solution requires a schematics workspace ID as input.**
...
1. It is recommended to have a COS Instance in the same region where the S/4HANA or BW/4HANA deployment is planned, as copying the files onto the lpar will be faster.
...
5. The **'ibmcloud_cos_configuration'** variable must be set correctly based on the folder structure created.
   ...
   "cos_region": IBM Cloud Object Storage instance bucket region, for example, **eu-gb**

"cos_bucket_name": The name of the COS bucket

"cos_hana_software_path": The folder path to HANA database binaries from the root of the bucket. For example, the value would be **"s4hana2022/HANA_DB"**

"cos_solution_software_path": The folder path to S/4HANA binaries from the root of the bucket. For example, the value would be **"s4hana2022/S4HANA_2022"**
